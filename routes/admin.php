...
Route::group(['prefix' => 'admin', 'middleware' => ['auth', 'admin', 'prevent-back-history']], function() {
    ...
    Route::resource('emis', EmiController::class);
        Route::controller(EmiController::class)->group(function () {
            Route::get('/emis', 'index')->name('emis.index');
            
            Route::get('/emis/show/{id}', 'view')->name('emis.show');
            Route::post('/bank/store', 'bankStore')->name('bank.store');
            
            Route::get('/emis/edit/{id}', 'edit')->name('emis.edit');
            Route::post('/emis/bankupdate/{id}', 'bankUpdate')->name('emis.bankupdate');
            
            Route::get('/emis/detailsEdit/{id}', 'detailsEdit')->name('emis.detailsedit');
            
            Route::post('/emis/bankdetailsUpdate/{id}', 'bankUpdateDetails')->name('emis.bankdetailsUpdate');
            
            Route::get('/emis/destroy/{id}', 'destroy')->name('emis.destroy');
            Route::get('/emis/destroy/{id}', 'destroyEmi')->name('emis.details.destroy');
            
            Route::post('/emis/store', 'emiStore')->name('emis.store');
    });
//Prebooking

Route::controller(PrebookingController::class)->group(function () {
        // All Orders
        Route::get('/all_prebooking', 'all_prebooking')->name('all_prebooking.index');
        Route::get('/show_prebooking/{id}', 'show')->name('all_prebooking.show');
        Route::get('/all_prebooking/destroy/{id}', 'destroy')->name('all_prebooking.destroy');
});

...
<div class="aiz-card-box h-auto bg-white py-3 hov-scale-img">
    <div class="position-relative h-140px h-md-200px img-fit overflow-hidden">
        ...
        <a href="{{ $product_url }}" class="d-block h-100">
            <img class="lazyload mx-auto img-fit" src="{{ static_asset('assets/img/placeholder.jpg') }}"
        ...
        @endif
            ...
            <!-- add to cart -->
        <a class="@if (in_array($product->id, $cart_added)) active @endif cart-btn absolute-bottom-left w-100 h-35px aiz-p-hov-icon text-white fs-13 fw-700 d-flex flex-column justify-content-center align-items-center" href="javascript:void(0)" @if (Auth::check()) onclick="showAddToCartModal({{ $product->id }})" @else onclick="showLoginModal()" @endif>
    ...
    <div class="p-2 p-md-3 text-left">
        ...
        <div class="fs-14 d-flex justify-content-center mt-3">
            ...
            @if ($product->auction_product == 0)
                ...
                <!-- price -->
                <div class="">
                    <span class="fw-700" style="color: #000;">{{ home_discounted_base_price($product) }}</span>
                </div>
            @endif
            @if ($product->auction_product == 1)
                <!-- Bid Amount -->
                <div class="">
                    <span class="fw-700" style="color: #000;">{{ single_price($product->starting_bid) }}</span>
